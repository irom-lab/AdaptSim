...
from pydrake.all import DiagramBuilder, Simulator, AddMultibodyPlantSceneGraph, Parser, DrakeVisualizer, LinearQuadraticRegulator, Saturation, PassThrough, Adder
...
class PendulumEnv():
    ...
    def _setup(self, task):
        ...
        for ind, joint_index in enumerate(self._joint_indices):
            ...
            joint.set_default_damping(damping)  # joint.damping()
        ...
        Q = np.identity(2)
        R = np.identity(1)
        self._actuation_input_port.FixValue(linearize_context, [0])
        ...
        torque_limiter = builder.AddSystem(
            Saturation(
                min_value=[-self.torque_limit], max_value=[self.torque_limit]
            )
        )

        # Add torque offset to realize true damping
        torque_offset = builder.AddSystem(PassThrough([0]))
    ...
    def reset(self, task=None):
        ...
        self.set_states(plant_context, [0])
    ...
    def reset_task(self, task=None):
        ...
        for ind, body_index in enumerate(self._body_indices):
            body = self._plant.get_body(body_index)
            body.SetMass(plant_context, task.true_m)

        # Record true damping - find offset
        self.b_offset = task.true_b - task.lqr_b

    @property
    def parameter(self):
        return np.array([self.task.true_m, self.task.true_b])
    ...
    def set_states(self, context, q, qdot=[0]):
